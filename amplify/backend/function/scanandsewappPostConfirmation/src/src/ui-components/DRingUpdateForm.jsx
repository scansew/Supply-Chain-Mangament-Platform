...
import { getDRing } from "../graphql/queries";
import { updateDRing } from "../graphql/mutations";
const client = generateClient();
export default function DRingUpdateForm(props) {
  const {
    id: idProp,
    dRing: dRingModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = dRingRecord
      ? { ...initialValues, ...dRingRecord }
      : initialValues;
    setDRingTotal(cleanValues.dRingTotal);
    setName(cleanValues.name);
    setNumber(cleanValues.number);
    setWebbing(cleanValues.webbing);
    setStrap(cleanValues.strap);
    setErrors({});
  };
  const [dRingRecord, setDRingRecord] = React.useState(dRingModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getDRing.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDRing
        : dRingModelProp;
      setDRingRecord(record);
    };
    queryData();
  }, [idProp, dRingModelProp]);
  React.useEffect(resetStateValues, [dRingRecord]);
  const validations = {
    dRingTotal: [],
    name: [{ type: "Required" }],
    number: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateDRing.replaceAll("__typename", ""),
            variables: {
              input: {
                id: dRingRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "DRingUpdateForm")}
      {...rest}
    >
      <TextField
        label="D ring total"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || dRingModelProp)}
