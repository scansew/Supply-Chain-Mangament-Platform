...
import { getRatchet } from "../graphql/queries";
import { updateRatchet } from "../graphql/mutations";
const client = generateClient();
export default function RatchetUpdateForm(props) {
  const {
    id: idProp,
    ratchet: ratchetModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = ratchetRecord
      ? { ...initialValues, ...ratchetRecord }
      : initialValues;
  ...
  const [ratchetRecord, setRatchetRecord] = React.useState(ratchetModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getRatchet.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRatchet
        : ratchetModelProp;
      setRatchetRecord(record);
    };
    queryData();
  }, [idProp, ratchetModelProp]);
  React.useEffect(resetStateValues, [ratchetRecord]);
  const validations = {
    name: [{ type: "Required" }],
    number: [],
    webbing: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateRatchet.replaceAll("__typename", ""),
            variables: {
              input: {
                id: ratchetRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "RatchetUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || ratchetModelProp)}
